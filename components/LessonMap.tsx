...
const LessonMap: React.FC<LessonMapProps> = ({ onSelectLesson, user, onPremiumClick }) => {
  ...
  const safeUnlockedSubjects = user?.unlockedSubjects || [SubjectType.GRAMMAR];

  const getNextPlayableLevel = () => {
    const lastUnlockedSubject = safeUnlockedSubjects[safeUnlockedSubjects.length - 1];
    const lastLevelObj = levels.find(l => l.subject === lastUnlockedSubject);
    return lastLevelObj || levels[0];
  };
  ...
  return (
    <div className="flex flex-col items-center w-full min-h-screen relative overflow-hidden bg-[radial-gradient(ellipse_at_top,_var(--tw-gradient-stops))] from-sky-200 via-sky-50 to-green-50 pb-32">
      ...
      <div className="relative z-20 w-full max-w-xl mx-auto pt-8 px-6 mb-8 flex justify-between items-center animate-fade-in-up">
        ...
        <button 
            onClick={() => { try { playSound.click(); } catch(e) {} onPremiumClick(); }}
            className={`px-3 py-1.5 rounded-full border shadow-sm flex items-center space-x-2 transition-all active:scale-95 cursor-pointer group ${user.isPremium ? 'bg-brand-dark border-brand-dark' : 'bg-white/80 backdrop-blur border-white hover:border-yellow-400 hover:bg-yellow-50'}`}
        >
            <span className={user.isPremium ? 'text-yellow-400' : 'text-yellow-500'}>👑</span>
            <span className={`font-bold text-sm ${user.isPremium ? 'text-white' : 'text-gray-700'}`}>{user.isPremium ? 'Membro PRO' : 'Grátis'}</span>
            {!user.isPremium && <div className="flex items-center ml-1 bg-brand-secondary text-white text-[10px] px-1.5 py-0.5 rounded font-bold animate-pulse">UPGRADE <ChevronRight size={10} /></div>}
        </button>
      </div>
      ...
      <div className="relative z-10 w-full max-w-md mx-auto flex-1 flex flex-col-reverse items-center justify-end py-10" style={{ minHeight: '800px' }}>
         ...
         {levels.map((level, index) => {
            ...
            const isUnlocked = safeUnlockedSubjects.includes(level.subject);
            const isPremiumLocked = level.premium && !user.isPremium;
            const isPlayable = isUnlocked && !isPremiumLocked;
            const isNext = isPlayable && index === safeUnlockedSubjects.length - 1; 

            return (
               <div key={level.id} className={`relative mb-12 transform transition-all duration-300 ${isNext ? 'scale-110' : 'scale-100'}`} style={{ transform: `translateX(${level.id % 2 === 0 ? '-30px' : (level.id === 1 || level.id === 4 || level.id === 7 ? '0px' : '30px')})` }}>
                  <button
                    onClick={() => { try { playSound.click(); } catch(e) {} if (isPremiumLocked) onPremiumClick(); else if (isPlayable) onSelectLesson(level.subject); }}
                    className={`w-20 h-20 md:w-24 md:h-24 rounded-full border-b-8 flex flex-col items-center justify-center shadow-xl transition-all relative group cursor-pointer ${isPlayable ? `${config?.color || 'bg-gray-400'} border-black/20 active:border-b-0 active:translate-y-2` : 'bg-white border-gray-300 cursor-not-allowed'} ${isPremiumLocked ? 'hover:scale-105 active:scale-95 cursor-pointer bg-gray-100' : ''}`}
                  >
                     <div className={`text-white drop-shadow-md ${isPlayable ? 'group-hover:scale-110 transition-transform' : 'opacity-30 text-gray-400'}`}>
                        {isPremiumLocked ? <Crown size={32} className="text-yellow-300 animate-pulse-slow" /> : !isUnlocked ? <Lock size={28} /> : config?.icon}
                     </div>
                     {isPlayable && !isPremiumLocked && (
                        <div className="absolute -bottom-10 flex space-x-1">
                           <Star size={12} className="text-yellow-400 fill-current" /><Star size={12} className="text-yellow-400 fill-current" /><Star size={12} className="text-gray-300" />
                        </div>
                     )}
                  </button>
                  ...
                  {isPlayable && index === safeUnlockedSubjects.length - 1 && ( 
      ...
      <div className="fixed bottom-24 md:bottom-8 right-6 z-50">
        <button onClick={handleQuickStart} className="bg-gradient-to-r from-brand-secondary to-orange-500 text-white p-4 rounded-full shadow-2xl border-4 border-white animate-bounce-sm hover:scale-110 transition-transform group flex items-center">
            <Play fill="currentColor" className="w-8 h-8" />
            <span className="max-w-0 overflow-hidden group-hover:max-w-xs group-hover:ml-2 transition-all duration-300 whitespace-nowrap font-bold">Continuar</span>
        </button>
      </div>
    </div>
  );
